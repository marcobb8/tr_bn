"""
Cython implementation of greedy search auxiliar function
...
from var_elim cimport PyFactorTree
...
cdef extern from "elim_tree.h":
    ...
    cdef cppclass Elimination_Tree nogil:
        ...
        int tw() nogil
        int tw(vector[int] evidence_vars) nogil
        int size() nogil
...
cdef int psearch_fts(Elimination_Tree etc, int [:] visited_mv, int [:] op_idx_mv, int [:,:] lop_mv, int num_op, int tw_bound, int cores, list evidence_vars = []):
    cdef int pos, xout, xin, i, etc2_tw, e
...
#    for i in range(num_op):
        ...
        if op_idx_mv[pos] == 0:
            ...
            etc2.optimize(xopt)
        if evidence_vars_c.size() > 0:
            etc2_tw = etc2.tw(evidence_vars_c) 
        else:
            etc2_tw = etc2.tw() 
        if etc2_tw<=tw_bound:          
            return pos
...
def search_first_tractable_structure(et, np.ndarray lop, np.ndarray op_names, int tw_bound, list forbidden_parents = None, bool constraint = False, int  cores=multiprocessing.cpu_count(), list evidence_vars = []):
    ...
    while (i <= first_valid):
        ...
        if visited_mv[i] == 0:
            ...
            if op_idx_mv[i] == 0:
                ...
                etc_cy2.remove_arc_opt(xout, xin)
                etc_cy2.add_arc_opt(xin, xout)
            if etc_cy2.tw(evidence_vars)<=tw_bound:
                classes=[]
                names=[]
                for j in range(et.nodes.num_nds):
                    names.append(et.nodes[j].name)
                ...
                etc_cy2.comp_to_et(et2)
#                print
#                print "etc_cy2.tw(evidence_vars): ", etc_cy2.tw(evidence_vars)
#                print "et2.tw(evidence_vars = evidence_vars): ", et2.tw(evidence_vars=evidence_vars)
#                evidence_vars
                return i, et2
        if constraint:
            if op_idx_mv[i] == 0:
