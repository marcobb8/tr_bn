...
cdef extern from "variable_elimination_data.h":
    ...
    void get_param_from_softmaxvar(vector[double] &x, vector[vector[double]] &parameters, const vector[int] &num_categories)    
...
#        Factor_Tree_data(const Factor_Tree_data& ft) except + #Very weird import error
        ...
        vector[double] compute_gradient(vector[double] &x,vector[vector[shared_ptr[vector[vector[int]]]]] &indicators_vector, vector[vector[shared_ptr[vector[vector[int]]]]] &indicators_evidence)
