...
#                print 'ch xj: ', xj, ndj.cluster.display()
    ...
    def tw(self,update=True, evidence_vars=[]):
        ...
        if update:
            ...
            for i in xrange(len(clusters)):
                cl = clusters[i].display() + [i]
                cl = list(set(cl).difference(evidence_vars))
                tw1 = len(cl) -1
                if tw1 > tw:
                    tw = tw1
        else:
            ...
            for i in range(self.nodes.num_nds):
                cl = self.nodes[i].cluster.display() + [i]
                cl = list(set(cl).difference(evidence_vars))
                tw1 = len(cl) -1
                if tw1 > tw:
                    tw = tw1
...
#      print 'old: ', old_cmplx, ', new: ', new_cmplx,', diff: ', new_cmplx - old_cmplx
    ...
    # Returns list of descendants, from the shallowest to the deepest
    def getDesc_py(self,int xi): ##CH##
        cdef Node_Set desc
        desc = Node_Set.__new__(Node_Set,self.nodes.size())
        self.getDesc_aux(xi,desc)
        return desc.display()
