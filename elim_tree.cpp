...
// Get tw after setting an evidence 
int  Elimination_Tree::tw(std::vector<int> evidence_vars){
    int mwidth = 0;
    std::vector<int> cluster_pruned;
    for (auto it = this->cluster_inner.begin(); it!=this->cluster_inner.end(); ++it){
        cluster_pruned = *it;
        auto it2 = std::set_difference (it->begin(), it->end(), evidence_vars.begin(), evidence_vars.end(), cluster_pruned.begin());
        cluster_pruned.resize(it2-cluster_pruned.begin());
        if (cluster_pruned.size()> mwidth){
            mwidth = cluster_pruned.size();   
