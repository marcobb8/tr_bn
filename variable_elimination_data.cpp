    ...
    for (auto it=0;it<indicators_vector.size();++it){
        ...
        for(unsigned i=0; i<ftd->data_size; ++i){
            if (prob_array[i]!=0){ll += log(prob_array[i]);}
                    ...
                    std::cout << "Warning: log_likelihood_parallel. Case " << i << " has probability 0. Min double is used to approximate the log-likelihood function\n";
                    ll += log(std::numeric_limits<double>::denorm_min());
                    }
        } 
        #pragma omp critical
        ll_sum += ll;
...
void get_param_from_softmaxvar(std::vector<double> &x, std::vector<std::vector<double>> &parameters, const std::vector<int> &num_categories){
   // Obtain gradient in the soft-max space (so that params sum 1) and update soft_max_variables
   unsigned x_pos = 0; 
   for (auto i = 0; i < parameters.size(); ++i){ 
        unsigned j=0; 
        unsigned nconf = parameters[i].size();       
        unsigned ncat = num_categories[i];
        while (j<nconf){
            long double sum_esm = 0; 
            // Obtain the sum of the exponentials of the soft-max parameters to later compute the gradient
            for (auto k=0; k<ncat; ++k){
                sum_esm += exp(x[x_pos+k]);
            }
            // Compute gradient and update soft_max values
            for (auto k=0; k<ncat; ++k){
                long double exp_sm_k = exp(x[x_pos+k]);
                if (sum_esm == 0){
                    #warning "get_param_from_softmaxvar: division by 0"
                } else{
                     parameters[i][j+k] = exp_sm_k / sum_esm;
                }
            }
            j += ncat;
            x_pos += ncat;
...
/*Compute gradient of the model given the parameters*/
std::vector<double> Factor_Tree_data::compute_gradient(std::vector<double> &x,std::vector<std::vector<std::shared_ptr<std::vector<std::vector<int>>>>> &indicators_vector, std::vector<std::vector<std::shared_ptr<std::vector<std::vector<int>>>>> &indicators_evidence){
    
        std::shared_ptr<Factor_Tree_data> ftd = this->copy();
        //Get param 2D vector from column_vector soft_max variables
        get_param_from_softmaxvar(x,ftd->parameters, ftd->num_categories);

        //Obtain partial derivatives of the CLL with respect to the parameters 
        //dCLL/dparamx = dLL(y,x;param)/dparamx - dll(x;param)/paramx
        std::vector<std::vector<double>> gradient(ftd->parameters.size()); 
        unsigned num_params = 0;
        for (auto i=0; i<gradient.size(); ++i){
            gradient[i].resize(ftd->parameters[i].size());
            num_params += parameters[i].size();
        } 

        #pragma omp parallel for shared(indicators_vector, indicators_evidence)
        for (auto it=0;it<indicators_vector.size();++it){
            std::shared_ptr<Factor_Tree_data> ftd2 = ftd->copy();
            //Obtain dLL(y,x;param)   
            ftd2->sum_compute_tree(ftd2->parameters,indicators_vector[it]);
            ftd2->distribute_evidence();
            std::vector<std::vector<double>> dll_yx = ftd2->se_leaves();
            //Obtain dLL(y,x;param)   
            ftd2->sum_compute_tree(ftd2->parameters,indicators_evidence[it]);
            ftd2->distribute_evidence();
            std::vector<std::vector<double>> dll_x = ftd2->se_leaves();
            #pragma omp critical  
            {for (auto i = 0; i < gradient.size(); ++i){
               for(auto j = 0; j < gradient[i].size(); ++j){
                    if (ftd2->parameters[i][j] == 0){
                        #warning "get_gradient_function: parameter equal to 0"
                    } else{
                        gradient[i][j] += dll_yx[i][j]/ftd2->parameters[i][j] - dll_x[i][j]/ftd2->parameters[i][j];
                    }
                }
            }}
        }
        ...
        unsigned x_pos = 0;
        std::vector<double> gradient_cv(num_params); 
        for (auto i = 0; i < gradient.size(); ++i){ 
            unsigned j=0; 
            unsigned nconf = gradient[i].size();       
            unsigned ncat = ftd->num_categories[i];
            while (j<nconf){
                long double sum_esm = 0; 
                // Obtain the sum of the exponentials of the soft-max parameters to later compute the gradient
                for (auto k=0; k<ncat; ++k){
                ...
                for (auto k=0; k<ncat; ++k){
                    long double exp_sm_k = exp(x[x_pos+k]);
                    double grad_soft_max = ((sum_esm-exp_sm_k)*exp_sm_k)/(sum_esm*sum_esm);
                    gradient_cv[x_pos+k] = grad_soft_max * gradient[i][j+k];
...
double Factor_Tree_data::compute_conditional_log_likelihood_function(std::vector<double> &x, std::vector<std::vector<std::shared_ptr<std::vector<std::vector<int>>>>> &indicators_vector, std::vector<std::vector<std::shared_ptr<std::vector<std::vector<int>>>>> &indicators_evidence){ 
    std::shared_ptr<Factor_Tree_data> ftd = this->copy();
    //Get param 2D vector from column_vector soft_max variables
    get_param_from_softmaxvar(x,ftd->parameters, ftd->num_categories);
    auto ll = ftd->log_likelihood_parallel(indicators_vector);
    auto fll = ftd->log_likelihood_parallel(indicators_evidence); 
    auto cll = ll  - fll;
    return cll;
}
